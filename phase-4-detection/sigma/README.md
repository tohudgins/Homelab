...
- **logsource** `product: windows`, `category: process_creation` → `<if_group>sysmon_event1</if_group>`.
- **field map** Sigma → Wazuh decoded field (`Image`→`win.eventdata.image`, `CommandLine`→`win.eventdata.commandLine`,
  `OriginalFileName`→`win.eventdata.originalFileName`, `ParentImage`, `IntegrityLevel`, …).
...
## Rules shipped in this first increment

| Sigma source | ATT&CK | Wazuh rules | Notes |
|---|---|---|---|
| `proc_creation_win_certutil_download.yml` | T1105 / T1027 | 100500, 100501 | **Verbatim upstream SigmaHQ rule** — proves the compiler ingests real community Sigma, not just hand-authored. |
| `proc_creation_win_process_discovery_tasklist.yml` | T1057 | 100502, 100503 | Authored here. |
| `proc_creation_win_system_owner_user_discovery.yml` | T1033 | 100504, 100505 | Authored here (whoami/quser/qwinsta). |

## Verification

- **Live fire (`purple-team.py`, ART on ws-01): 11/11.** T1057 (tasklist) and T1033 (whoami) — the Sigma-compiled
  rules — fire end-to-end alongside the nine pre-existing detections. Coverage map → **36 techniques, 14 validated**.
- **Rule logic (`sigma-selftest.py`): 9/9.** Emulates Wazuh's field AND/negate evaluation against sample events;
  asserts each rule fires on a true positive and stays quiet on look-alikes (e.g. benign `certutil -hashfile`
  does **not** trip the download rule — precision, no false positive).
...
## Add a detection
1. Drop a Sigma `.yml` in `rules/` (Windows process_creation).
