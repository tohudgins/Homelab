...
| 35 | [T1558.003 – Kerberoasting](https://attack.mitre.org/techniques/T1558/003/) + [T1078 – Valid Accounts](https://attack.mitre.org/techniques/T1078/) | 100420, 100421 | dc-01 (Samba KDC audit) | ✅ verified TP live — **AD honeytoken tripwire** (decoy `svc-sqladmin`): 100420 fires L14 on a TGS-REQ for the decoy SPN, 100421 L14 on any auth as it. **~zero FP** (no legit process touches the decoy); a single targeted roast trips it while volume rule 100031 stays silent — closes 100031's documented evasion. See [`deception/`](deception/README.md) (2026-09-05) |
| 36 | [T1071.004 – Application Layer Protocol: DNS](https://attack.mitre.org/techniques/T1071/004/) + [T1048.003 – Exfil Over Alternative Protocol](https://attack.mitre.org/techniques/T1048/003/) | Suricata 9100010 | rtr-01 (Suricata NSM) | ✅ verified TP live (100 alerts) — **DNS tunneling** detection: long encoded query names to REDTEAM at volume. Exercised with a live `iodine` tunnel (fs-01→atk-01); Zeek confirmed 360 queries/one zone, 100% unique, mean 127c/max 681c. Paired with the `hunt-dns-tunnel.py` analytic (scored it 94). Fills the **Exfiltration** tactic. See [`phase-6-nsm/dns-tunneling.md`](../phase-6-nsm/dns-tunneling.md) (2026-09-05) |
| 37 | [T1486 – Data Encrypted for Impact](https://attack.mitre.org/techniques/T1486/) | 100430, 100431 | fs-01 (realtime FIM) | ✅ verified TP live — **ransomware canary**: a decoy `/srv/finance-records/` (valuable-looking bait, no legit access) under realtime FIM. 100430 L12 on any change to a canary file (13× in the test); 100431 L13 on a **burst** (8+ changes/20s = mass-encryption fingerprint). Exercised with a benign ransomware sim (overwrite→`.locked`+note). Fills the **Impact** tactic. Alert-only (no AR). See [`deception/`](deception/README.md) (2026-09-05) |
| 38 | [T1190 – Exploit Public-Facing Application](https://attack.mitre.org/techniques/T1190/) (+ [T1595.002 – Active Scanning](https://attack.mitre.org/techniques/T1595/002/)) | Suricata 9100020–9100024 + Wazuh 100440, 100441, 100442 | rtr-01 (inline Suricata) → siem-01 | ✅ **verified TP live** (2026-09-07) — **DMZ web-attack detection** for OWASP Juice Shop: SQLi (URI + POST body), XSS, path-traversal, and scanner-UA sigs, escalated to labeled T1190 SIEM alerts (100440, now L13) with a burst-correlation rule (100442). Closes the lab's last gap — the DMZ had attack surface + log ingestion but **no detection**. Finding 1: ET Open web rules key on `$HTTP_PORTS` and were blind to Juice Shop's port 3000 (fixed in the `router` role). **Finding 2 (first live-fire, two real bugs):** Suricata's PCRE matched the *raw* URL-encoded request, not the decoded one (`%27%20OR%201=1--` never satisfies a pattern written for `' OR 1=1`) — fixed with a `url_decode` transform on every URI/body-based rule. Then, even with Suricata firing correctly, Wazuh silently dropped the alert to a **rule-precedence collision**: atk-01's own IP is on the threat-intel CDB blocklist from unrelated exercises, and that rule (100211, L12) outranked 100440 (L10) whenever both matched the same event — raised 100440/100441 to L13 to win. Same class of gotcha as Sigma rules needing to clear the stock L3 discovery band, just against a different competing rule. Exercise: [`phase-5-offense/web-attacks/`](../phase-5-offense/web-attacks/); writeup [`03-web-attack-juiceshop-t1190.md`](../phase-5-offense/attack-detect-writeups/03-web-attack-juiceshop-t1190.md) (2026-09-06, re-verified 2026-09-07) |
| 39 | [T1047 – Windows Management Instrumentation](https://attack.mitre.org/techniques/T1047/) | 100515 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest); live-fire attempted 2026-09-07, genuinely unresolved after 5 ruled-out causes.** `WmiPrvSE.exe` spawning a shell = remote command execution over WMI. Switched from `nxc --exec-method wmiexec` (reliably times out — a real but *different*, since-diagnosed nxc-vs-lab incompatibility, not this rule's problem) to `impacket-wmiexec` directly, which **succeeds cleanly** (`whoami` returns correctly) and produces the **exact expected telemetry** — confirmed via `archives.json`: `Image=cmd.exe`, `ParentImage=...\wbem\WmiPrvSE.exe`. The rule still never fires. Systematically ruled out: **(1) Defender** — no detection logged for any attempt. **(2) Windows Firewall** — traffic reaches the host and the WMI call completes (fixed the same session, see the note below the table). **(3) A syntax/load error** — the ruleset loads clean, no errors in `ossec.log`. **(4) Rule precedence** — tested at level 16 (Wazuh's actual maximum, confirmed via a real, self-inflicted, ~90s manager outage from first trying an *invalid* level 20 — reverted immediately, `xmllint`-checked before every redeploy since); still didn't fire, ruling out a competing higher-level rule. **(5) A stale ruleset** — a full clean manager restart didn't change the outcome. **(6, unconfirmed)** the one lead not yet chased down: the same `ProcessGuid` sometimes arrives at the manager with **completely empty `eventdata`** in `archives.json` (seen twice per WMI attempt, alongside two populated copies) — a live duplicate-delivery race that may mean the rule engine evaluates a different copy than the one with the data. Left genuinely open rather than forced. |
| 40 | [T1021.006 – Remote Services: WinRM](https://attack.mitre.org/techniques/T1021/006/) | 100516 (Sigma) | ws-01 (Sysmon EID1) | ✅ **verified TP live (2026-09-07), after a listener + a real rule fix.** No WinRM listener existed on ws-01 by design (`windows` role deliberately uses SSH, not WinRM); stood one up specifically to exercise this technique (`winrm quickconfig`, Tyler's call). First attempt still didn't fire — `nxc winrm -x whoami` uses **`WinRShost.exe`** (the Windows Remote Shell host, for raw single-command execution) as the parent, not `wsmprovhost.exe` (which only appears for `Enter-PSSession`-style interactive sessions) — a real Sigma-rule coverage gap the rule's author hadn't anticipated. Fixed by widening `ParentImage` to accept either. Also found in passing: `nxc winrm` defaults to **domain** auth and fails silently against a local account unless `--local-auth` is passed. Re-verified clean after both fixes. Fills a genuine 2nd Lateral Movement technique (was only T1021.002). |
| 41 | [T1569.002 – System Services: Service Execution](https://attack.mitre.org/techniques/T1569/002/) | 100513, 100514 (Sigma) | ws-01 (Sysmon EID1) | ⚠️ **logic verified (sigma-selftest + precision); live-fire blocked by Defender, not by tooling** — attempted 2026-09-07 via `impacket-psexec -service-name PSEXESVC` (the *actual* PsExec tool; `nxc`'s `--exec-method` doesn't offer a `psexec` choice at all in this nxc version — `ad-validate.py` had an invalid argument and had never really attacked anything until fixed the same session). Reaching ws-01 needed two more infra fixes first (see the note below the table). Once it did, the dropped service binary (`PSEXESVC.exe`, matching the exact name the rule expects) is detected and quarantined by Defender as **`VirTool:Win32/RemoteExec!pz`** before the service can run — same class of finding as T1105/T1003.001-comsvcs: the endpoint control is the outer layer, this rule is the layer that catches the technique wherever that control is weakened, disabled, or bypassed. (Aside: impacket-psexec randomizes the binary/service name *unless* `-service-name` is passed — a tool run without that flag would evade this rule's literal `PSEXESVC` match even if Defender allowed it, a separate, real coverage note.) See writeup [`04-lateral-movement-wmi-winrm-psexec.md`](../phase-5-offense/attack-detect-writeups/04-lateral-movement-wmi-winrm-psexec.md) |

> **Infrastructure fixed while chasing #39–41 live (2026-09-07)** — not a detection, but the reason the
> attacks could reach ws-01 at all. ws-01's Windows Firewall had the **entire "File and Printer Sharing" and
> "Windows Management Instrumentation (WMI)" rule groups disabled**, for every profile — a side effect of the
> "debloated" base image, never noticed because nothing had tried live inbound lateral movement before. Its
> network was also misclassified `Public` instead of `DomainAuthenticated` (stale NLA detection from boot),
> and even after fixing both, the enabled SMB-In rule was scoped `RemoteAddress: LocalSubnet` — invisible to
> REDTEAM traffic routed in from a different segment, so `dc-01` (same subnet) could reach it while `atk-01`
> (routed) couldn't. Fixed all three: enabled the rule groups, set the profile to `Private`, widened the scope
> to `Any`. Separately, `impacket-psexec` against `localadmin` (a non-`Administrator` local account) failed to
> write to `ADMIN$`/`C$` until `LocalAccountTokenFilterPolicy=1` was set — the standard, documented Microsoft
> fix for UAC's remote token-filtering of local accounts. **None of this is codified into the `windows`
> Ansible role yet** — a live-only, `ssh`-applied fix, not idempotent, not survives-a-rebuild. Flagged as a
> follow-up rather than left silent.
| 42 | [T1560.001 – Archive Collected Data: via Utility](https://attack.mitre.org/techniques/T1560/001/) | 100517, 100518, 100519 (Sigma) | ws-01 (Sysmon EID1 + PS 4104) | 🔧 **logic verified (sigma-selftest 31/31 incl. precision), live-fire pending** — **fills the Collection tactic** (previously empty): archive utilities (rar/7-Zip/WinRAR, 100517/100518) and fileless `Compress-Archive` (100519) = the "stage-before-exfil" step that sits right upstream of the DNS-exfil detection (#36). Exercise: run `7z a`/`Compress-Archive` on ws-01. **Sigma-compiled** (2026-09-06) |
...
| 44 | [T1003.001 – LSASS Memory (command-line angle)](https://attack.mitre.org/techniques/T1003/001/) | 100525, 100526 (Sigma) | ws-01 (Sysmon EID1) | ⚠️ **logic verified (sigma-selftest 38/38 incl. precision); live-fire blocked by Defender, not by PPL** — attempted 2026-09-07: `rundll32.exe comsvcs.dll, MiniDump …` never spawns. Microsoft Defender detects and removes the exact command line as **`Trojan:Win32/RundllLolBin.AF`** (ThreatID 2147793100, confirmed via `Get-WinEvent` on the Windows Defender/Operational log, action=Remove) before the child process is created, so there's no Sysmon EID1 to catch — same defense-in-depth class as T1105 certutil (§ below): the endpoint control is the outer layer, this rule is the layer that catches the technique wherever that control is weakened, disabled, or bypassed. **This is a stricter finding than the one the rule was written for** — it was added specifically to survive LSASS PPL blocking the *read* (the gap #12 left open, stock EID10 rule 92900 inspection-only), but in this lab's default config Defender blocks the *launch* first, so PPL is never even reached. Exercise: [`phase-5-offense/credential-access/lsass-dump.ps1`](../phase-5-offense/credential-access/lsass-dump.ps1) (fails the same way; not removed from the purple-team battery, never added — see `phase-5-offense/purple-team/tests.json`'s `_comment_not_in_battery`). **Sigma-compiled** (2026-09-06) |
