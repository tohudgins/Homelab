...
| 36 | [T1071.004 – Application Layer Protocol: DNS](https://attack.mitre.org/techniques/T1071/004/) + [T1048.003 – Exfil Over Alternative Protocol](https://attack.mitre.org/techniques/T1048/003/) | Suricata 9100010 → Wazuh **100443** | rtr-01 (Suricata NSM) | ✅ verified TP live (100 alerts) — **DNS tunneling** detection: long encoded query names to REDTEAM at volume. Exercised with a live `iodine` tunnel (fs-01→atk-01); Zeek confirmed 360 queries/one zone, 100% unique, mean 127c/max 681c. Paired with the `hunt-dns-tunnel.py` analytic (scored it 94). Fills the **Exfiltration** tactic. **Gap found and fixed running the capstone for real (2026-09-07): 9100010 is only a Suricata sid, and there was no labeled Wazuh rule for it at all** — a fresh live tunnel (60 pings, 0% loss, sid fired 126×) surfaced in Wazuh only as the generic threat-intel CDB hit (rule 100210), and only because atk-01's IP happens to be blocklisted from an unrelated exercise; remove that coincidence and this technique was invisible to the SIEM. Fixed the same way the T1190 rule was built: a labeled child of stock rule 86601 (`100443`, level 13, keyed on the Suricata signature text) — verified firing live (`firedtimes:100`). See [`phase-6-nsm/dns-tunneling.md`](../phase-6-nsm/dns-tunneling.md) (2026-09-05, corrected 2026-09-07) |
...
| 39 | [T1047 – Windows Management Instrumentation](https://attack.mitre.org/techniques/T1047/) | 100515 (Sigma), **100527 (fix)** | ws-01 (Sysmon EID1) | ✅ **root-caused and fixed live (2026-09-12), after 5 causes ruled out on 2026-09-07 left it genuinely unresolved.** `WmiPrvSE.exe` spawning a shell = remote command execution over WMI; `impacket-wmiexec` produces the exact expected telemetry (`Image=cmd.exe`, `ParentImage=...\wbem\WmiPrvSE.exe`) but 100515 never fired. **Root cause:** stock rule **92069** ("Windows management instrumentation started a process", level 0, *unanchored* `parentImage` match on `WmiPrvSE.exe`) silently wins Wazuh's one-rule-per-event resolution against 100515 — 100515 is anchored on the same top-level `if_group=sysmon_event1` as 92069 rather than chained as 92069's *child*, so it's never even considered once 92069 matches first (92069's own child, 92070, only escalates for a PowerShell target — a `cmd.exe` child gets silently absorbed at level 0). **Confirmed empirically**: temporarily neutralizing 92069 on the live manager made 100515 fire immediately on the identical live event; reverting and adding a proper fix instead confirmed it again. **Fix:** `sigma-to-wazuh.py` can't express an `if_sid` chain to a specific stock rule (it anchors every `process_creation` Sigma rule uniformly on `if_group=sysmon_event1`), so this is a genuinely new, hand-written escalation rule — **100527**, `if_sid=92069` + the same child-image regex as 100515 — the same idiom as 100020/100050/100070 elsewhere in this catalog. 100515 stays in place (still Sigma-verified by `sigma-selftest.py`, documents the intended detection logic); 100527 is what actually fires live. Verified with 3 separate live `impacket-wmiexec` runs, clean manager restart in between, debug logging on to trace the exact mechanism. Wired into `ad-validate.py`'s WMI scenario (`"rules": ["100515", "100527"]`). |
...
| 41 | [T1569.002 – System Services: Service Execution](https://attack.mitre.org/techniques/T1569/002/) | 100513, 100514 (Sigma) | ws-01 (Sysmon EID1) | ⚠️ **logic verified (sigma-selftest + precision); live-fire blocked by Defender, not by tooling** — attempted 2026-09-07 via `impacket-psexec -service-name PSEXESVC` (the *actual* PsExec tool; `nxc`'s `--exec-method` doesn't offer a `psexec` choice at all in this nxc version — `ad-validate.py` had an invalid argument and had never really attacked anything until fixed the same session). Reaching ws-01 needed two more infra fixes first (see the note below the table). Once it did, the dropped service binary (`PSEXESVC.exe`, matching the exact name the rule expects) is detected and quarantined by Defender as **`VirTool:Win32/RemoteExec!pz`** before the service can run — same class of finding as T1105/T1003.001-comsvcs: the endpoint control is the outer layer, this rule is the layer that catches the technique wherever that control is weakened, disabled, or bypassed. **Checked and closed 2026-09-23**: run `impacket-psexec` for real *without* `-service-name` against ws-01 — it randomizes both the service name (`aSOd`) and the binary (`hEUJklFD.exe`), which does evade 100513/100514's literal `PSEXESVC` match exactly as expected, confirmed live (100513/100514 stayed silent on this run). But it doesn't leave the technique undetected: **stock rule 92650** (`0840-win_event_channel.xml`, not previously known to be relevant here) independently fired at level 12, tagged both T1021.002 and T1569.002 — it matches *any* newly-installed service whose binary sits directly in `%systemroot%`, regardless of service/binary name, which is exactly the structural fingerprint of an admin-share-dropped service binary. No new rule needed; the practical gap doesn't exist, it was just undocumented. See writeup [`04-lateral-movement-wmi-winrm-psexec.md`](../phase-5-offense/attack-detect-writeups/04-lateral-movement-wmi-winrm-psexec.md) |
...
> fix for UAC's remote token-filtering of local accounts. **Now codified into the `windows` Ansible role**
> (`roles/windows/tasks/main.yml`) as idempotent check-then-fix tasks — syntax-checked and lint-clean;
> not yet re-converged against a live ws-01 to confirm `changed=0` on a second run.
