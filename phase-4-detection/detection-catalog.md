...
| 39 | [T1047 – Windows Management Instrumentation](https://attack.mitre.org/techniques/T1047/) | 100515 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest 26/26 incl. precision); live-fire attempted 2026-09-07, unresolved** — `WmiPrvSE.exe` spawning a shell = remote command execution over WMI (`nxc winrm`... `wmiexec`). First live attempt reached ws-01 (a fresh `WmiPrvSE.exe` provider host spawns, confirmed via syscollector) but never produced a **child** process, and `nxc` reports "NETBIOS connection... timed out" — Sysmon never sees a `ParentImage=WmiPrvSE.exe` event to fire on. Not Defender (no detection logged for this attempt) and not Windows Firewall (SMB/WMI-In rule groups fixed the same session, see #3 below) — most likely the DCOM callback for the `Win32_Process.Create` result, which needs the ephemeral RPC port range Windows negotiates per-call; not yet root-caused. Genuinely open, not glossed over. |
| 40 | [T1021.006 – Remote Services: WinRM](https://attack.mitre.org/techniques/T1021/006/) | 100516 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest); live-fire blocked — no WinRM listener exists on ws-01, by design.** Phase 7's `windows` Ansible role deliberately manages ws-01 over **SSH, not WinRM** (`group_vars/windows.yml`: "avoids standing up a WinRM listener + certificate"). Confirmed 2026-09-07: port 5985 doesn't even accept a TCP connection. Standing up a listener just to exercise this one technique is a real option but changes the host's documented management posture — a call for Tyler, not made unilaterally. Fills a 2nd Lateral Movement technique on paper (was only T1021.002) but isn't independently exercisable as the lab is currently built. |
