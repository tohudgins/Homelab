...
| 39 | [T1047 – Windows Management Instrumentation](https://attack.mitre.org/techniques/T1047/) | 100515 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest 26/26 incl. precision), live-fire pending** — `WmiPrvSE.exe` spawning a shell = remote command execution over WMI (impacket `wmiexec.py` / `nxc --exec-method wmiexec`). **Sigma-compiled** (2026-09-06) |
| 40 | [T1021.006 – Remote Services: WinRM](https://attack.mitre.org/techniques/T1021/006/) | 100516 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest), live-fire pending** — any child of `wsmprovhost.exe` = inbound WinRM / PS-Remoting execution (evil-winrm / `nxc --exec-method winrm`). Fills a 2nd Lateral Movement technique (was only T1021.002). **Sigma-compiled** (2026-09-06) |
| 41 | [T1569.002 – System Services: Service Execution](https://attack.mitre.org/techniques/T1569/002/) | 100513, 100514 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest + precision), live-fire pending** — `PSEXESVC.exe` running or spawning a child = PsExec-style service execution (Sysinternals/impacket psexec, `nxc --exec-method psexec`). **Sigma-compiled** (2026-09-06). See writeup [`04-lateral-movement-wmi-winrm-psexec.md`](../phase-5-offense/attack-detect-writeups/04-lateral-movement-wmi-winrm-psexec.md) |
