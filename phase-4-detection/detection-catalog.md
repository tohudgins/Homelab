...
| 43 | [T1490 – Inhibit System Recovery](https://attack.mitre.org/techniques/T1490/) | 100520–100524 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest 35/35 incl. precision), live-fire pending** — the classic ransomware pre-encryption step: `vssadmin delete shadows` / `wmic shadowcopy delete` / `wbadmin delete` / `bcdedit` recovery-disable. **Deepens the Impact tactic** alongside the fs-01 ransomware canary (#37) — covers Impact from the recovery-destruction angle, not just file-change. Exercise: run `vssadmin delete shadows /all /quiet` on ws-01. **Sigma-compiled** (2026-09-06) |
| 44 | [T1003.001 – LSASS Memory (command-line angle)](https://attack.mitre.org/techniques/T1003/001/) | 100525, 100526 (Sigma) | ws-01 (Sysmon EID1) | 🔧 **logic verified (sigma-selftest 38/38 incl. precision), live-fire pending** — **makes T1003.001 live-fireable**, the gap #12 left open (stock EID10 rule 92900 was inspection-only because LSASS PPL blocked the dump). This catches the `comsvcs.dll MiniDump` LOLBin + `rundll32`→lsass on the *command line*, which is logged even when PPL denies the read. Exercise: [`phase-5-offense/credential-access/lsass-dump.ps1`](../phase-5-offense/credential-access/lsass-dump.ps1). **Sigma-compiled** (2026-09-06) |
