...
> **Status: all 8 build phases (0–7) complete.** The detection catalog runs to **55 ATT&CK techniques** — 52 verified end-to-end by an automated attack→detect→PASS harness (`purple-team.py`/`ad-validate.py`), including T1047 WMI and T1098.007 AD group-membership logging (both root-caused and fixed 2026-09-12 after prior investigations left them genuinely unresolved), T1548.003 sudo/GTFOBins privesc (the lab's first Linux-native offense/detection pair), T1003.002 SAM dump via RemoteRegistry (closed 2026-09-24 after a full-repo coverage audit found it was the one technique with an attack writeup and zero detection), and T1003.003 NTDS dump (closed 2026-09-25 — Samba has no ntds.dit to steal locally, but an unscoped replication pull hits the exact same rule as DCSync, honestly co-tagged rather than duplicated). The remaining 3 are deliberate, documented exclusions from the automated battery — a confirmed Defender defense-in-depth block on the exact telemetry the detection would need (LSASS PPL, certutil, PsExec) — not gaps — spanning 13 of ATT&CK's 14 tactics ([`phase-4-detection/detection-catalog.md`](phase-4-detection/detection-catalog.md); [`technique-index.md`](phase-4-detection/attack-coverage/technique-index.md) links every one of the 55 to wherever it's actually explained), plus an SCA before/after remediation pass. A capstone end-to-end intrusion emulation (`phase-5-offense/apt-scenario/`) chains 7 of those phases into one detected kill chain, scored **11/12** on a genuine fresh full re-run with all 7 non-scan hosts up (the sole miss is LSASS comsvcs, confirmed Defender-blocked before it can even run) ([`run-scenario.sh --verify`](phase-5-offense/apt-scenario/run-scenario.sh)). Phase 5 maps AD attack paths in BloodHound and executes one from a Kali box back into detection — with an honest writeup of where Linux offensive tooling does and doesn't work against a Samba DC ([`phase-5-offense/attack-detect-writeups/`](phase-5-offense/attack-detect-writeups/)). Phase 6 pairs Suricata + Zeek NSM; Phase 7 is full Ansible IaC (`site.yml` converges all seven hosts at `changed=0`). A [CI pipeline](.github/workflows/ci.yml) statically validates every committed artifact on each push — Ansible (syntax-check + `ansible-lint`), the Sigma detections, the Wazuh rules/decoders, Python and shell tooling, and a `gitleaks` secret scan. A [docs site](https://tohudgins.github.io/Homelab/) ([`mkdocs.yml`](mkdocs.yml)) publishes the whole catalog, and the [ATT&CK coverage map loads directly into Navigator](phase-4-detection/attack-coverage/README.md#view-it) from a link — no manual upload. Remaining portfolio work is a walkthrough video. Full build plan and design rationale: [`docs/`](docs/).
...
| 4 | Detection engineering | ✅ Complete (53 techniques across 13 tactics, 49 verified end-to-end live-fire, `phase-4-detection/detection-catalog.md`) |
